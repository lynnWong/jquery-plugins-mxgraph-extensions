define(function(require, exports, module) {
	...
	UI.initListener = function(){
		var navNames=['welcome','document','download'],containerNames = ["plugins","extensions","tools"];
		$("div.modal-btn").hover(function(){
			$("div#welecom_carousel>ol.carousel-indicators>li:eq("+$(this).attr("for")+")").click();
		}).on("click",function(){
			$("div.welcome").hide();
			$("div.container."+containerNames[$(this).attr("for")]).removeClass("hide").show();
		});
	...
	UI.initPlugins = function(){
		...
		new PluginUI({
			...
			code:'$("#zoom").zoomPanel({'+"<br>"+
					'	scale : 1,'+"<br>"+
					'	html:"显示图片的img节点",'+"<br>"+
					'	zoomChange : function(scale){'+"<br>"+
					'	    var $img = $("#example-zoomPanel img.example-show-img");'+"<br>"+
					'	    var width = 180*scale;'+"<br>"+
		...
		new PluginUI({
			...
			doCode:function(){
				$("#example-tableLoader>table").tableLoader({
						data : [{name:"西瓜",type:"水果",msg:"边看恐怖片边吃"},
								{name:"德普",type:"帅哥",msg:"歪国著名奇葩"},
								{name:"程序猿",type:"不明",msg:"只想呵呵了"},
								{name:"程序猿",type:"吃货",msg:"对 他们是吃货"},
								{name:"凑数的1",type:"吃货",msg:"凑数的能干什么呢"},
								{name:"凑数的2",type:"吃货",msg:"凑数的能干什么呢"},
								{name:"凑数的3",type:"吃货",msg:"凑数的能干什么呢"},
								{name:"凑数的4",type:"吃货",msg:"凑数的能干什么呢"}],
						paging : true,
						pageSize : 3,
						columns : [{
							label : "名称",
	...
	PluginUI.prototype.initUI = function(){
		this.option.$container.append("<h3>"+this.option.title+"</h3>")
		.append("<p>"+this.option.description+"</p>")
		.append("<div class='highlight'><label>html</label><pre><code class='language-html ruby'></code></pre></div>").find("code.language-html").text(this.option.example).end()
		.append("<div class='highlight'><label>javascript</label><pre><code class='language-js ruby'>"+this.option.code+"</code></pre></div>")
		.append("<label>案例</lable>")
		.append("<div class='bs-example'>"+this.option.example+"</div>");
